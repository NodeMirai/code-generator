...
import { LogColor, ErrorType } from "./constant";
...
let count = 0;
class PageSource {
  methodAttrSet: Set<string> = new Set();      // 所有方法属性的集合，会自动声明在页面中
  attrCodeListSet: Set<string> = new Set();        // 属性集合，这里有问题。。。重复属性只获取了一次
  nativeComponentSet: Set<string> = new Set(); // 原生组件集合，用于import原生部分
  thirdComponentSet: Set<string> = new Set(); // 原生组件集合，用于import原生部分
  componentSet: Set<string> = new Set();       // 组件名称集合，用于当前页面代码的import部分
    ...
    traverse(ast, {
      ...
      ImportDeclaration: path => {
        ...
        if (tc && tc[0].value === "import") {
          this.componentSet.forEach(csName => {
            path.insertAfter(
              /**
          ...
          if (nativeComponentPath && this.nativeComponentSet.size > 0) {
            // 原生组件导入
            path.insertAfter(
              astUtilBase.getAstByCode(
                `import {${Array.from(this.nativeComponentSet).join(
                  ", "
                )}} from '${nativeComponentPath}'`
              )
            );
          }
          if (thirdComponentPath && this.thirdComponentSet.size > 0) {
            // 第三方组件导入
            path.insertAfter(
              astUtilBase.getAstByCode(
                `import {${Array.from(this.thirdComponentSet).join(
                  ", "
                )}} from '${thirdComponentPath}'`
              )
            );
          }
      ...
      ClassMethod: path => {
        ...
        if (key.name === "render") {
          ...
          if (this.attrCodeListSet.size !== 0) {
            block.unshiftContainer(
              "body",
              astUtilBase.getAstByCode(
                `const { ${Array.from(this.attrCodeListSet).join(
                  ", "
                )} } = this.props`
              )
            );
    ...
    cs.propList.forEach((prop: string | Prop) => {
      if (typeof prop === "string") {
        if (strUtil.isMethodAttr(prop)) {
          let propTmp = prop;
          if (this.methodAttrSet.has(prop)) {
            prop = prop + ++count;
          }
          this.methodAttrSet.add(prop);
          jsxAttrCodeStr += `${propTmp}={this.${prop}} `;
        } else {
          this.attrCodeListSet.add(prop);
        ...
        if (strUtil.isMethodAttr(name)) {
          let propTmp = name;
          if (this.methodAttrSet.has(name)) {
            name = name + ++count;
          }
  ...
  initChildrenAst(
    ...
    children.forEach((cs: ComponentSource) => {
      ...
      switch (cs.name[0]) {
        case '$':
          this.nativeComponentSet.add(cs.name.slice(1));
          break;
        case '&':
          this.thirdComponentSet.add(cs.name.slice(1));
          break;
        default:
