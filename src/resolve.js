...
function generatorAstFromConfig(innerConfig, outConfig) {
    ...
    const { componentPath, resolveComponentPath, nativeComponentPath, modelPath, outPath } = innerConfig
        ...
        const { key } = path.node
            if (key.name === 'render') {
                ...
                children.forEach(item1 => {
                    ...
                    console.log('item1.name', item1.name, 'jsxAttrCodeStr', jsxAttrCodeStr)
                ...
                console.log('childrenCode.join()', childrenCode.join(';'))
